...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    ...
    description: `Learn more about ${teamMember.name}, our ${teamMember.position.toLowerCase()} with expertise in digital marketing and strategy.`,
  }
}
...
export default function TeamMemberPage({ params }: { params: { slug: string } }) {
  ...
  const prevIndex = currentIndex > 0 ? currentIndex - 1 : teamMembers.length - 1
      ...
      <div className="max-w-6xl mx-auto py-16 px-4">
        <div className="flex flex-col md:flex-row gap-8 md:gap-16">
          ...
          <div className="w-full md:w-2/3">
            <h1 className="text-4xl md:text-5xl font-light text-gray-700 mb-6">{teamMember.name}</h1>
            <p className="text-gray-600">
              Etiam porta sem malesuada magna mollis euismod. Integer posuere erat a ante venenatis dapibus posuere
              velit aliquet. Fusce dapibus, tellus ac cursus commodo, tortor mauris condimentum nibh, ut fermentum massa
              justo sit amet risus. Nulla vitae elit libero, a pharetra augue. Sed posuere consectetur est at lobortis.
      ...
      <div className="border-t border-b border-gray-200 py-4">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex justify-between items-center">
            <Link href={`/team/${prevMember.slug}`} className="flex items-center text-blue-500 hover:text-blue-700">
            ...
            <Link href={`/team/${nextMember.slug}`} className="flex items-center text-blue-500 hover:text-blue-700">
      ...
      <div className="bg-blue-400 py-16 text-white">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
            <div className="mb-8 md:mb-0">
              <div className="flex items-center mb-6">
                ...
                <Link href="#" className="text-sm border-b border-white hover:border-transparent">
          ...
          <div className="flex justify-center mt-12">
            {[...Array(8)].map((_, i) => (
              <div key={i} className={`w-2 h-2 rounded-full mx-1 ${i === 0 ? "bg-white" : "bg-blue-200"}`} />
            ))}
          </div>
        </div>
      </div>
